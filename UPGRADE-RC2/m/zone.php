...
class Model_Zone extends Model_Base
{
	...
	function validate($save = true)
	{
		...
		if (($save && empty($_POST['name'])) || ($save && empty($_POST['owner'])) || 
            empty($_POST['refresh']) || empty($_POST['retry']) || 
            empty($_POST['expire']) || empty($_POST['ttl']) || 
            empty($_POST['pri_dns']) || empty($_POST['sec_dns'])) {
		...
		if (!is_numeric($_POST['refresh']) || !is_numeric($_POST['retry']) || 
            !is_numeric($_POST['expire']) || !is_numeric($_POST['ttl'])) {
			$this->registry->template->error .= "Refresh, Retry, Expire etc can only ".
                "consist of numbers <br />\n";
		}

		// Validate ip adresses
		$ipRegex = "^([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])".
		...
		if (!empty($_POST['transfer'])) {
			...
			$transferRegex = '/^(([1-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])'.
                '(\.([0-9]|[1-9][0-9]|1[0-9][0-9]|2[0-4][0-9]|25[0-5])){3}\; ?)+$/';

			if (preg_match($transferRegex, $_POST['transfer']) == 0)
				$this->registry->template->error .= "Allow-transfer was not using the ".
                    "right syntax <br />\n";
		}
	...
	function saveReg()
	{
		if (!$this->save(array('name' => $_POST['name'], 'refresh' => $_POST['refresh'], 
                               ...
                               'owner' => $_POST['owner'], 'serial' => date("Ymd") . '00', 
            ...
            if (!empty($_POST['www'])) {
                if (!$this->save(array('zone' => $id, 'host' => '@', 'type' => 'A', 
                                       'pri' => 0, 'destination' => $_POST['www']), 'records')) {
                    // Delete zone record because of error
                    $this->delete($id);
                    return false;
                } else {
                    if (!$this->save(array('zone' => $id, 'host' => 'www', 'type' => 'A', 
                                           'pri' => 0, 'destination' => $_POST['www']), 
                                     'records')) {
                        $this->delete($id);
            ...
            if (!empty($_POST['mail'])) {
                if (!$this->save(array('zone' => $id, 'host' => 'mail', 'type' => 'A', 
                                       'pri' => 0, 'destination' => $_POST['mail']), 'records')) {
                    // Rollback 
                    $this->delete($id);
                    return false;
                } else {
                    if (!$this->save(array('zone' => $id, 'host' => '@', 'type' => 'MX', 
                                           'pri' => 5, 'destination' => $_POST['mail']), 
                                     'records')) {
                        // Rollback
                        $this->delete($id);
                        $this->delete("zone = ?", array($id), 'records');
